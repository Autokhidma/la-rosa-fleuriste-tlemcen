...
router.post('/products', auth, (req, res) => {
  ...
  const info = db
    .prepare(`INSERT INTO products (name, category, description, special, price, images, sizes, colors, best_seller, active, sort_order)
              VALUES (@name, @category, @description, @special, @price, @images, @sizes, @colors, @best_seller, @active, @sort_order)`)
    .run(p);
...
router.put('/products/:id', auth, (req, res) => {
  ...
  const info = db
    .prepare(`UPDATE products SET name=@name, category=@category, description=@description, special=@special,
              ...
              active=@active, sort_order=@sort_order WHERE id=@id`)
...
const EDITABLE_SETTINGS = [
  ...
  'facebook', 'address', 'map_url', 'hours', 'currency',
