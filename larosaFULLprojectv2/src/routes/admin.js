 ...
 * livraison / paramètres. Toutes les routes (sauf /login) exigent une session valide.
 */
...
// ---------- Auth ----------
// Jetons signés HMAC, sans état : fonctionnent aussi en serverless (Vercel),
// où chaque instance a sa propre base éphémère.
...
router.post('/login', (req, res) => {
  ...
  if (!password || !verifyPassword(String(password), getSetting('admin_password_hash'))) {
...
router.post('/password', auth, (req, res) => {
  ...
  setSetting('admin_password_hash', hashPassword(String(new_password)));
  res.json({ ok: true });
});
...
router.get('/stats', auth, (_req, res) => {
  ...
  res.json({
    orders_total: q('SELECT COUNT(*) n FROM orders').n,
    orders_new: q("SELECT COUNT(*) n FROM orders WHERE status = 'nouvelle'").n,
    orders_today: q("SELECT COUNT(*) n FROM orders WHERE date(created_at) = date('now')").n,
    revenue_month: q(
      "SELECT COALESCE(SUM(total),0) s FROM orders WHERE strftime('%Y-%m', created_at) = strftime('%Y-%m','now') AND status != 'annulée'"
    ).s,
    products_count: q('SELECT COUNT(*) n FROM products WHERE active = 1').n,
    best_sellers: q('SELECT COUNT(*) n FROM products WHERE best_seller = 1 AND active = 1').n,
...
router.get('/orders', auth, (req, res) => {
  ...
  res.json(db.prepare(sql).all(...params).map((o) => ({ ...o, items: JSON.parse(o.items) })));
});
...
router.put('/categories/:id', auth, (req, res) => {
  ...
  try {
    db.prepare('UPDATE categories SET name = ?, emoji = ?, icon = ?, sort_order = ?, active = ? WHERE id = ?')
    ...
    if (newName !== cat.name)
      db.prepare('UPDATE products SET category = ? WHERE category = ?').run(newName, cat.name);
    res.json({ ok: true });
  } catch {
...
router.delete('/categories/:id', auth, (req, res) => {
  const cat = db.prepare('SELECT * FROM categories WHERE id = ?').get(req.params.id);
  if (!cat) return res.status(404).json({ error: 'Catégorie introuvable' });
  // Les produits orphelins passent dans « Autres » (créée au besoin)
  const count = db.prepare('SELECT COUNT(*) n FROM products WHERE category = ?').get(cat.name).n;
  if (count > 0) {
    db.prepare(`INSERT INTO categories (name, emoji, sort_order) VALUES ('Autres', '🎁', 99)
                ON CONFLICT(name) DO NOTHING`).run();
    db.prepare('UPDATE products SET category = ? WHERE category = ?').run('Autres', cat.name);
  }
...
// ---------- Événements spéciaux ----------
router.get('/events', auth, (_req, res) => {
  res.json(db.prepare('SELECT * FROM events ORDER BY sort_order ASC').all());
});
...
router.put('/events/:id', auth, (req, res) => {
  ...
  const b = req.body || {};
  db.prepare('UPDATE events SET name = ?, emoji = ?, banner = ?, color1 = ?, color2 = ?, particles = ?, active = ? WHERE id = ?')
    .run(
...
// ---------- Livraison ----------
router.get('/zones', auth, (_req, res) => {
  res.json(db.prepare('SELECT * FROM delivery_zones ORDER BY fee ASC').all());
});
...
router.get('/settings', auth, (_req, res) => {
  const out = {};
  EDITABLE_SETTINGS.forEach((k) => (out[k] = getSetting(k)));
  res.json(out);
});

router.put('/settings', auth, (req, res) => {
  for (const [k, v] of Object.entries(req.body || {})) {
    if (EDITABLE_SETTINGS.includes(k)) setSetting(k, String(v ?? '').slice(0, 2000));
  }
  res.json({ ok: true });
});
