...
### 💾 Persistance des données (obligatoire pour tout sauvegarder)

La base utilise **libSQL** : un simple fichier en local, et **Turso** (base
SQLite hébergée, gratuite) en production. Sans Turso sur Vercel, le disque est
éphémère (`/tmp`) et **les produits/commandes ajoutés sont perdus** au prochain
redéploiement.

Pour que tout se sauvegarde sur Vercel :
1. Créez une base gratuite : https://turso.tech → `turso db create larosa`
2. `turso db show larosa --url` et `turso db tokens create larosa`
3. Dans Vercel → Settings → Environment Variables, ajoutez
   `TURSO_DATABASE_URL` et `TURSO_AUTH_TOKEN`, puis redéployez.

Les images téléversées vont dans `/tmp` sur Vercel (éphémères) — pour des
photos permanentes, utilisez une URL d'image ou un hébergeur persistant.
