...
async function notifyNewOrder(order) {
  // Les variables d'environnement priment (persistantes sur Vercel, même sans base Turso)
  if ((process.env.NOTIFY_ENABLED || (await getSetting('notify_enabled', '1'))) === '0') return { skipped: true };

  const text = buildOrderText(order);
  const report = {};

  const webhookUrl = (process.env.N8N_WEBHOOK_URL || (await getSetting('n8n_webhook_url', ''))).trim();
  if (webhookUrl) {
    try {
      await post(webhookUrl, {
        event: 'new_order',
        shop: await getSetting('shop_name', 'La Rosa Fleuriste Tlemcen'),
        text,
        order: {
          ...order,
  ...
  const botToken = (process.env.TELEGRAM_BOT_TOKEN || (await getSetting('telegram_bot_token', ''))).trim();
  const chatId = (process.env.TELEGRAM_CHAT_ID || (await getSetting('telegram_chat_id', ''))).trim();
  if (botToken && chatId) {
    try {
