...
router.get('/products', async (req, res) => {
  ...
  res.json((await db.prepare(sql).all(...params)).map(parseProduct));
});

router.get('/products/:id', async (req, res) => {
  const row = await db.prepare('SELECT * FROM products WHERE id = ? AND active = 1').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'Produit introuvable' });
  res.json(parseProduct(row));
});

router.get('/categories', async (_req, res) => {
  res.json(await db.prepare('SELECT * FROM categories WHERE active = 1 ORDER BY sort_order ASC, name ASC').all());
});

router.get('/events', async (_req, res) => {
  res.json(await db.prepare('SELECT * FROM events WHERE active = 1 ORDER BY sort_order ASC').all());
});

router.get('/zones', async (_req, res) => {
  res.json(await db.prepare('SELECT * FROM delivery_zones WHERE active = 1 ORDER BY fee ASC').all());
});
...
router.post('/orders', async (req, res) => {
  try {
    ...
    let zone = null;
    if (zone_id) zone = await db.prepare('SELECT * FROM delivery_zones WHERE id = ? AND active = 1').get(zone_id);
    const deliveryFee = zone ? zone.fee : 0;
    const total = subtotal + deliveryFee;

    const year = new Date().getFullYear();
    const seq = (await db.prepare("SELECT COUNT(*) AS n FROM orders WHERE order_number LIKE ?").get(`LR-${year}-%`)).n + 1;
    ...
    res.status(201).json({
      ...
      whatsapp: await getSetting('whatsapp'),
    });
  } catch (err) {
